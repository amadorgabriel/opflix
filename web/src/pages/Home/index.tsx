...
export default class App extends Component {
  ...
  filtrarPorCategoria = (event) => {
    fetch(
      "http://192.168.4.199:5000/api/Lancamentos/filtrarCategoria/" +
        event.target.value
    )
      .then((data) => data.json())
      .then((res) => {
  ...
  ordenarPorData = (event) => {
    ...
    fetch(
      "http://192.168.4.199:5000/api/Lancamentos/filtrarData/" +
        event.target.value
    )
      .then((data) => data.json())
      .then((res) => {
  ...
  Favoritar = (event) => {
    ...
    Axios.get(
      "http://192.168.4.199:5000/api/Lancamentos/favoritar/" +
        event.target.value,
      config
    ).then((data) => {
      if (data.status === 200) {
        ...
        <main className="footerBasic paddingBottom">
          ...
          <div className="divFiltro">
            ...
            <div className="divChoices">
              <select
                className="filtroI"
                onChange={this.filtrarPorCategoria}
                name=""
                id=""
              >
              ...
              <button to="/" className="btnFav" onClick={this.listarFavoritos}>
                {" "}
                Favoritos{" "}
              </button>
              ...
              return (
                <section key={element.idLancamento} className="lancamento">
                  <img
                    className="imgFilme"
                    src={
                      element.fotoLanc != undefined
                        ? element.fotoLanc
                        : element.idLancamentoNavigation.fotoLanc
                    }
                  />
                  <article className="infoFilmes">
                    <h3>
                      {element.titulo != null
                        ? element.titulo
                        : element.idLancamentoNavigation.titulo}
                    </h3>
                    <p>
                      <strong className="strg">Sinopse:</strong>
                      {element.sinopse != null
                        ? element.sinopse
                        : element.idLancamentoNavigation.sinopse}
                    </p>
                    <p>
                      <strong className="strg">Duração:</strong>
                      {element.duracao != null
                        ? element.duracao
                        : element.idLancamentoNavigation.duracao}
                    </p>
                    <p>
                      <strong className="strg">Data Lançamento:</strong>
                      {element.dataLancamento != null
                        ? element.dataLancamento
                        : element.idLancamentoNavigation.dataLancamento}
                    </p>
                    <p>
                      <strong className="strg">Categoria:</strong>
                      {element.idCategoriaNavigation != undefined
                        ? element.idCategoriaNavigation.nome
                        : "nulo"}
                    </p>
                    <p>
                      <strong className="strg">Conteúdo:</strong>
                      {element.idTipoConteudoNavigation != undefined
                        ? element.idTipoConteudoNavigation.nome
                        : "nulo"}
                    </p>

                    <button
                      className="btnFavorito"
                      id={element.idLancamento}
                      value={element.idLancamento}
                      onClick={this.Favoritar}
                    >
                      {" "}
                      {this.state.msgFavoritado}{" "}
                    </button>
                  </article>
                </section>
              );
